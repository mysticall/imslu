...
function ip_add($db, $ip) {
  ...
  if ($ip['stopped'] == 'n' && ($user['free_access'] == 'y' || $expire > $now || substr($user['created'],0,10) == date('Y-m-d'))) {
    ...
    $cmd = "$SUDO $IMSLU_SCRIPTS/functions-php.sh ip_allow '{$ip['ip']}' '{$user['serviceid']}'";
    shell_exec($cmd);
  }

  if ($OS == 'Linux') {
    // Add tc filter for IP
    $cmd = "$SUDO $IMSLU_SCRIPTS/functions-php.sh tc_filter_add '{$ip['ip']}' '{$ip['userid']}'";
    shell_exec($cmd);
  }
}
...
function ip_remove($db, $ip) {
  ...
  if (!empty($ip['username']) && ($ip['protocol'] == 'PPPoE')) {
    ...
    $cmd = "$SUDO $IMSLU_SCRIPTS/functions-php.sh ip_rem '{$ip['ip']}' '{$ip['vlan']}'";
    shell_exec($cmd);
  }

  // Stop internet access for IP
  $cmd = "$SUDO $IMSLU_SCRIPTS/functions-php.sh ip_stop {$ip['ip']}";
  shell_exec($cmd);

  if ($OS == 'Linux') {
    // Remove tc filter for IP
    $cmd = "$SUDO $IMSLU_SCRIPTS/functions-php.sh tc_filter_delete {$ip['ip']}";
    shell_exec($cmd);
  }
}
